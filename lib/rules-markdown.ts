...
export const RULES_SECTIONS: RulesSection[] = [
  ...
  },
  {
    ...
    kind: "invariant",
    items: [
      "하드코딩 색(hex, rgb, 임의 OKLCH 값 등)을 직접 쓰지 않는다 — 항상 시맨틱 토큰만 사용한다: themes/ 의 프리셋 27키(bg-background, text-primary 등)와 globals.css 의 :root/.dark 가 소유한 보조 층(sidebar-*, risk-*, heatmap-l-*/heatmap-text-*)이다. 보조 층은 닫힌 목록이 아니라 확장되는 층이다 — 기존 토큰 중 어느 것으로도 표현할 수 없는 성격(연속 값 강도 등)이면 lib/<층>-tokens.ts 로 층을 새로 만들고 PROFILE_CSS_VAR_KEYS 에 합류시킨다.",
      ...
      "위험등급 토큰은 gain/loss와 같은 도메인 관례 토큰이지만 층의 위치가 다르다 — gain/loss는 프리셋 27키에 들어 있고 모든 프리셋이 같은 값을 적는 방식이고(themes/*.ts가 SSOT), risk-*는 sidebar-*처럼 프리셋 키에서 빠져 globals.css의 :root/.dark에만 정의된다(lib/risk-tokens.ts가 SSOT). 그래서 [data-theme=\"*\"] 블록에서 risk-*를 재정의하지 않는다 — 브랜드에 맞춰 경보를 초록으로 바꾸는 식의 오버라이드는 표준 위반이다.",
      ...
      "값의 강도를 면색으로 얹는 격자(matrix-heatmap 등)는 chart-1~5 나 risk-* 를 빌려 쓰지 않고 --heatmap-l-0~4 / --heatmap-text-0~4 층(lib/heatmap-tokens.ts)을 쓴다 — 셀 배경은 oklch(from var(--primary) var(--heatmap-l-<n>) c h) 로 프로필의 --primary 에서 파생시키고 값(hue/chroma)을 이 층이 고정하지 않는다. risk-*·sidebar-* 와 같은 층이라 [data-theme=\"*\"] 블록에서 재정의하지 않는다.",
