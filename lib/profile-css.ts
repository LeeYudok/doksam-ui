...
export function generateProfileCode(profile: BrandProfile): string {
  ...
  const densityCss = [
    ...
    "  gap: var(--control-gap-sm);",
    "}",
    '[data-density]\n  [data-slot="button"]:is([data-size="default"], [data-size="lg"]):has(\n    [data-icon="inline-start"]\n  ) {',
    "  padding-inline-start: calc(var(--control-px) * 0.8);",
    "}",
    '[data-density]\n  [data-slot="button"]:is([data-size="default"], [data-size="lg"]):has(\n    [data-icon="inline-end"]\n  ) {',
    "  padding-inline-end: calc(var(--control-px) * 0.8);",
    "}",
    '[data-density]\n  [data-slot="button"][data-size="sm"]:has([data-icon="inline-start"]) {',
    "  padding-inline-start: calc(var(--control-px) * 0.75);",
    "}",
    '[data-density] [data-slot="button"][data-size="sm"]:has([data-icon="inline-end"]) {',
    "  padding-inline-end: calc(var(--control-px) * 0.75);",
    "}",
    '[data-density]\n  [data-slot="button"][data-size="xs"]:has([data-icon="inline-start"]) {',
    "  padding-inline-start: calc(var(--control-px-xs) * 0.75);",
    "}",
    '[data-density] [data-slot="button"][data-size="xs"]:has([data-icon="inline-end"]) {',
    "  padding-inline-end: calc(var(--control-px-xs) * 0.75);",
    "}",
    '[data-density] [data-slot="button"][data-size="icon"] {',
    "  width: var(--control-h);",
    "  height: var(--control-h);",
    "}",
    '[data-density] [data-slot="button"][data-size="icon-xs"] {',
  ...
  // app/globals.css 의 @theme inline 층(#43)과 동일 값 — --stack-gap 을 이름 있는
  // spacing 키로 승격해 gap-stack·space-y-stack·py-stack 유틸리티를 만든다.
  // @theme 안에 있어야 Tailwind 가 유틸리티를 생성하므로 [data-density] 스코프
  // 밖, 별도 @theme inline 블록으로 방출한다.
  const spacingCss = [
    "/* 섹션 세로 간격 스페이싱 키 — gap-stack·space-y-stack·py-stack 유틸리티용 */",
    "@theme inline {",
    "  --spacing-stack: var(--stack-gap, 1.5rem);",
    "}",
  ].join("\n");
  ...
  const personalityCss = [
    ...
    '[data-personality-surface="shadow"]',
    "  :is(",
    '    [data-slot="card"],',
    ...
    "}",
    '[data-personality-surface="shadow"] :is([data-slot="input"], [data-slot="textarea"]) {',
    "  box-shadow: inset 0 1px 2px color-mix(in oklch, var(--foreground) 8%, transparent);",
    "}",
    '[data-personality-surface="flat"]',
    "  :is(",
    '    [data-slot="card"],',
    '    [data-slot="popover-content"],',
    '    [data-slot="select-content"],',
  ...
  const cornerCss = [
    ...
    // 파생 반경 재계산 — @theme 의 --radius-sm/md/lg/xl/2xl/3xl/4xl 는 :root 에
    // 방출되지 않고 유틸리티에 인라인되므로, data-corner 가 걸린 요소에서 다시
    // 선언해야 components/ui/button.tsx 의 rounded-[min(var(--radius-md),10px)]
    // 등이 유효한 값을 얻는다(app/globals.css 의 [data-corner] 블록과 동일).
    "[data-corner] {",
    ...
    ...(corner.control === corner.surface
      ? []
      : [
          `[data-corner="${corner.name}"] :is([data-slot="button"], [data-slot="input"], [data-slot="select-trigger"], [data-slot="textarea"], [data-slot="toggle"], [data-slot="toggle-group-item"]) {`,
          "  border-radius: var(--radius-control);",
          "}",
          // 캘린더 내비 버튼은 Button 컴포넌트가 아니라 buttonVariants() 를 className
          // 으로만 쓰기 때문에 data-slot="button" 이 찍히지 않아 위 규칙이 안 걸린다.
          // 슬롯 대신 캘린더 하위 버튼을 직접 잡는다. 크기는 격자 정렬 때문에 그대로 둔다.
          `[data-corner="${corner.name}"] [data-slot="calendar"] button {`,
          "  border-radius: var(--radius-control);",
          "}",
        ]),
  ].join("\n");
  ...
  const typeContrastCss = [
    ...
    "  --text-xs: calc(0.75rem * var(--heading-scale));",
    "  --text-sm: calc(0.875rem * var(--heading-scale));",
