...
GitLab 클론의 `content/rules.mdx` 도 생성물입니다(#72) — 규칙 절을 고쳤으면 이
레포(SSOT, CI 가 Node 22)에서 `pnpm gen:rules-mdx <gitlab 클론>/content` 를 돌려 그
...
GitLab 은 완성된 MDX 파일만 받는 구조로 그 제약을 피합니다.

**`rules.en.mdx` 는 생성물이 아니라 수동 유지 파일입니다.** SSOT 에 영문 본문이 없어
생성기가 낼 수 있는 것은 절 제목과 번역 대기 플레이스홀더뿐이고, 그것으로 GitLab 의
손번역을 덮으면 번역이 전량 소실됩니다. 그래서 기본 실행은 ko 만 쓰고, en 골격은
`--en`(기존 파일이 있으면 `--force` 까지)을 명시해야만 씁니다. 절이 추가됐는데 영문이
비어 있는 상태는 `scripts/check-gitlab-drift.mjs` 가 두 레포의 절 제목 집합을 비교해
잡습니다.
