...
- `components/ui/` 는 `components.json` 의 style(`radix-nova`)로 설치한 상류 원본이다 — 손으로 고치지 않고, 다른 스타일의 프리미티브로 갈아끼우지도 않는다(밀도·모서리 축이 그 값을 전제한다). 대조 절차는 `scripts/shadcn-upstream.mjs`, 기록은 `components/ui/upstream.manifest.json`. 커스텀 조합은 `components/` 또는 `components/patterns/` 에.
...
- `primitive` — shadcn CLI 로 설치한 `components/ui/` 저수준 빌딩블록. 갱신은 `scripts/shadcn-upstream.mjs` 절차로만 하고(결과는 `upstream.manifest.json` 에 기록), 화면 작업 중에는 수정하지 않는다. 카탈로그가 직접 배포하므로 프리미티브를 추가·수정했으면 `pnpm registry:sync` 를 돌린다(#57).
...
| 종류 | 위치 | 표기 |
|---|---|---|
| shadcn 프리미티브 | `components/ui/<name>.tsx` | kebab-case, 화면 작업 중 수정 금지(갱신은 `scripts/shadcn-upstream.mjs` 절차로만) · 추가·수정 시 `pnpm registry:sync` |
