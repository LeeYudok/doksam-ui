...
export const LAYOUT_ARCHETYPES: LayoutArchetype[] = [
  ...
  },
  {
    ...
    description:
      ...
      "이 뼈대가 금지하는 것은 '다른 목적지로 전환하는 크롬'이지 정보 표시 자체가 아니라서, 클릭해도 다른 화면으로 이동시키지 않는 시스템 상태바·공지·법적/보안 고지 같은 " +
      "정보성 표시는 카드 위·아래에 둘 수 있습니다(#101).",
    suitedFor: [
      "로그인·OTP 등 인증 화면",
      "결제·삭제처럼 되돌릴 수 없는 확인 화면",
      "접근 차단·점검 안내·초대 수락",
      "시스템 바·공지·보안 고지가 붙는 사내 업무포털 로그인",
    ],
    navigation:
      "지속되는 내비게이션 없음 — 상단 브랜드 표기, 주 액션 버튼 1개와 보조 액션 1개, 하단 이탈용 보조 링크가 전부. " +
      "목적지를 바꾸지 않는 정보성 상태바·공지·보안 고지는 내비게이션이 아니므로 이 예산에 포함되지 않는다.",
    skeleton:
      "상단 브랜드 표기 외 크롬 없음(클릭해도 다른 화면으로 이동하지 않는 시스템 상태바·공지·보안 고지 같은 정보성 표시는 허용) → " +
      "화면 정중앙 카드 하나에 과제 하나(주 액션 1 + 보조 액션 1) → 하단 보조 링크. 사이드바·상단 메뉴·하단 탭바처럼 다른 목적지로 이동하는 내비게이션은 없다.",
    shell: "집중형 셸",
    templates: ["passkey-auth", "portal-login"],
    avoidWhen: [
      ...
      "상단·하단 정보 영역에 다른 화면으로 이동하는 링크(내비게이션)를 넣어야 하는 경우 — 그 순간부터는 정보성 표시가 아니라 내비게이션이라 focus-task 가 아니다",
    ],
    icon: TargetIcon,
  },
];
