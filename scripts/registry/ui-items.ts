...
import { REPO_ROOT, collectSpecifiers, packageOfSpecifier, resolveSpecifier, type RegistryItem } from "./closure"
...
export function readManifest(): Record<string, ManifestEntry> {
  const manifest = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, UI_DIR, "upstream.manifest.json"), "utf8")) as {
...
function describe(name: string, manifest: Record<string, ManifestEntry>): string {
  const entry = manifest[`${name}.tsx`]
  const groups = entry?.groups?.length ? entry.groups.join(", ") : null
  if (entry?.customized && groups) {
    return `${titleOf(name)} 프리미티브. 카탈로그가 렌더하는 것과 같은 파일을 배포한다 — 상류 레지스트리 원문과는 ${groups} 가 다르다(밀도·모서리·컨트롤 스케일 축이 이 값을 전제한다).`
  }
  return `${titleOf(name)} 프리미티브. 카탈로그가 렌더하는 것과 같은 파일을 배포한다 — bare 이름으로 상류에서 받으면 프리셋·상류 버전에 따라 내용이 갈린다.`
}
 ...
 */
export function buildUiItem(name: string, manifest = readManifest(), ranges = versionRanges()): RegistryItem {
  ...
  while (queue.length) {
    ...
    for (const spec of collectSpecifiers(source)) {
      ...
      const sibling = target.match(new RegExp(`^${UI_DIR}/([a-z0-9-]+)\\.tsx$`))
      if (sibling) {
  ...
  const item: RegistryItem = {
    ...
    type: "registry:ui",
    title: titleOf(name),
    description: describe(name, manifest),
  } as RegistryItem
...
export function expectedUiItems(): RegistryItem[] {
  ...
  return houseUiNames().map((name) => buildUiItem(name, manifest, ranges))
}
