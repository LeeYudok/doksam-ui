 ...
 * 에서 계산한다(`lib/profile-registry-css-vars.ts`). `description` 의
 * `radius=<값>` 부분도 `profiles/index.ts` 의 `profile.radius` 로 다시
 * 맞춘다 — 어드버서리얼 리뷰에서 5개 중 4개가 실제 radius 와 어긋나 있는
 * 것으로 드러났다(설명 문장 나머지는 손으로 쓴 캐비어트라 건드리지 않는다).
 * 프로필의 `theme`/`radius` 를 바꾸거나 테마 프리셋 자체를 고쳤으면 이것을
...
import { computeProfileRegistryCssVars, withSyncedRadiusDescription } from "../../lib/profile-registry-css-vars.ts"
...
for (const profile of BRAND_PROFILES) {
  ...
  if (item.description) {
    item.description = withSyncedRadiusDescription(item.description, profile.radius)
  }
  changed++
}
